% xcompound_EXP1 shows the effect of xcompounding several uncertain transformations in a row
%
...
kmax = 101;
for i = 1 : imax
    T{i,1} = xvec2tran( zeros(6,1) );
    for k = 2:kmax
        T{i,k} = xvec2tran( cholSigma * randn(6,1) )*Tbar * T{i,k-1};

    end
end

% Propagate the uncertainty using second- and fourth-order methods
T_est{1} = xvec2tran( zeros(6,1) );
Sigma_est1{1} = zeros(6);  % second order covariance
Sigma_est2{1} = zeros(6); % fourth order covariance
for k = 2:kmax
       
   % Second-order method
   [Ttemp,Sigmatemp] = xcompound( Tbar, Sigma, T_est{k-1}, Sigma_est1{k-1}, 1 );
   T_est{k} = Ttemp;
   Sigma_est1{k} = Sigmatemp;

   % Fourth-order method
   [Ttemp,Sigmatemp] = xcompound( Tbar, Sigma, T_est{k-1}, Sigma_est2{k-1}, 2 );
...
%print -depsc xcompound_exp1.eps
%saveas(hf, 'xcompound_exp1.fig')
